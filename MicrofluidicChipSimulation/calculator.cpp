...
void Calculator::setedgelength(int x, double leng)
{
    edges[x].leng = leng;
}
...
//函数功能：将方程加入到方程组中
//参数含义：tmp，新的方程
void Calculator::addrect(vector<double> &tmp){
    rect.push_back(tmp);
}
...
int Calculator::getdirline(int x, int y, int dir){
    int e;
    int sum = x*n+y;
    if (dir == 0){
        if (y==0){
...
//参数含义：x，节点的横坐标；y节点的纵坐标；dir，询问的节点方向。
bool Calculator::existdir(int x, int y, int dir)
{
...
void Calculator::recursionline(int x, int y, int dir, vector<double> &tmp,int end){
    int e = getdirline(x,y,dir);
    if (e>EDGESUM-6)
        return;
    int nodeno = x*n+y;
...
bool Calculator::recursionrect(int x, int y, vector<double> &tmp,int end){
    int xx,yy;
    for(int i=1; i<5; i++)
    {
...
void Calculator::findline(int x, int y,int t){
...
//参数含义：x1，输入管道编号。
void Calculator::findrect(int x1){
    vector<double> tmp(EDGESUM+1,0);
...
//函数功能：计算a和b的最大公约数。
//参数含义：。
double Calculator::GreatestCommonDivisor(double a, double b)
{
    double t;
    if (a < b)
    {
...
//函数功能：计算a和b的最小公倍数。
//参数含义：。
double Calculator::LeastCommonMultiple (double a, double b)
...
vector<double> Calculator::caluconspeed(int num, vector<double>&length, int i1, int i2, int o1, int o2, int o3)
{
    rect.clear();
    n = num;
    EDGESUM = 2*n*n-2*n+5;
